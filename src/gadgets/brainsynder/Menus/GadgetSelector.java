...
public class GadgetSelector {
    ...
    public static void openMenu(Player player, int page) {
        Inventory inv = Bukkit.createInventory(null, 54, "Gadgets: " + page + "/" + Core.get().getGadgetPageMaker().getMaxPages());
        ...
        if (Core.get().getGadgetPageMaker().getMaxPages() > page) {
        ...
        for (String s : Core.get().getGadgetPageMaker().getPage(page)) {
