...
public class Gadget_WinterBreeze extends Gadget {
    public Gadget_WinterBreeze() {
        super(10, "winter_breeze", Material.ICE, (byte) 0);
    }
    ...
    public void run(final Player p) {
        ...
        BukkitRunnable iceMaker = new BukkitRunnable() {
            @Override
            public void run() {
                if (!ice.isValid()) {
                    cancel();
                ...
                for (Block blocks : BlockUtils.getBlocksInRadius(ice.getLocation(), 4, false)) {
                    BlockUtils.setToRestore(Core.get(), blocks, (i == 1) ? Material.ICE : Material.PACKED_ICE, (byte) 0, 20 * 3);
                    ParticleRef.sendParticle(blocks.getLocation(), (i == 1) ? Material.ICE : Material.PACKED_ICE, (byte) 0, 1.0F, 1.0F, 1.0F, 5);
                    i = rand.nextInt(2) + 1;
                    ice.remove();
                    trailer.clear();
                }
        ...
        iceMaker.runTaskLater(getPlugin().getGadgetPlugin(), 20);
    }
